...
OPENCV4_VERSION = 2d3a760be769fcc48e7f1fb6744adb653843f2fb
OPENCV4_SITE = $(call github,sophgo,sophon_opencv,$(OPENCV4_VERSION))
...
extern_lib_path = ${OPENCV4_SRCDIR}/extern_lib
ifeq ($(BR2_PACKAGE_OPENCV4_WITH_FFMPEG),y)
OPENCV4_CONF_OPTS += -DWITH_FFMPEG=ON -DHAVE_FFMPEG=ON
OPENCV4_DEPENDENCIES += ffmpeg bzip2
OPENCV4_CONF_OPTS += -DFFMPEG_INCLUDE_DIRS="${extern_lib_path}/3rdparty/ffmpeg/include;          \
...
OPENCV4_CONF_OPTS += -DFFMPEG_LIBRARY_DIRS="${extern_lib_path}/3rdparty/ffmpeg/lib;              \
...
define OPENCV4_INSTALL_EXTERN_CMDS
 cp $(@D)/extern_lib/3rdparty/libisp/lib/lib*.so* $(TARGET_DIR)/usr/lib
 cp $(@D)/extern_lib/3rdparty/libisp/lib/soc/lib*.so* $(TARGET_DIR)/usr/lib
# cp $(@D)/extern_lib/3rdparty/libdrm/lib/lib*.so* $(TARGET_DIR)/usr/lib
endef
OPENCV4_POST_INSTALL_TARGET_HOOKS += OPENCV4_INSTALL_EXTERN_CMDS
